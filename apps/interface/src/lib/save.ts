// Save-as-agent through M11 (M15 §2.4) — reuse the registry's existing semantics; invent none.
//
...
// returns 409 `agent_exists` and the COCKPIT composes that into an add-version (M11 seam #1: the
// raw API does not auto-route). We mirror that compose here. Version bumping is the author's job
