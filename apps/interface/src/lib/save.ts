...
// returns 409 `agent_exists`, composed here into an add-version (the raw API does not
// auto-route). Version bumping is the author's job via the editable `version` field —
// re-publishing identical (id, version) content is an idempotent 200; different content under
// the same version is a 409 `version_conflict` ("bump the version"), surfaced verbatim. The
// server strips `view`, hashes, and returns the persisted contentHash.
