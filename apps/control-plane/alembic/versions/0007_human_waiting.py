"""run.awaiting_node for the M14 durable human wait (m14.md §1.1)

Revision ID: 0007_human_waiting
Revises: 0006_trigger
Create Date: 2026-06-23

M14 adds the four additive-lowering node types (``human``/``subgraph``/``loop``/``map``). Only the
...
* The ``waiting`` run status is **additive at the value level only** — ``run.status`` is a plain
  ...
  matters for ``waiting`` is behavioral: M9's reconciliation sweep already touches only
  ``created``/``streaming``, so a ``waiting`` run is excluded from it for free (m14.md §1.1 / §4).
* ``run.awaiting_node`` — the one new column. When a run pauses at a ``human`` node on
  ...
  nullable, exactly like M5's ``graph_id`` and M12's ``trigger_id`` — a plain breadcrumb, not an FK.
...
def upgrade() -> None:
    # The waiting-node breadcrumb (§1.1): which human node a `waiting` run is paused at. NULL when
