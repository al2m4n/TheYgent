...
* ``span`` — the timeline rows; one per emitted span (a run-root span, a node span, or a phase
  ...
  - ``id`` is a **deterministic composite** (``{run_id}:{node_id}[:{phase}][:#{branch}]``), not a
    random ULID. This is the load-bearing half of the resume-idempotency rule: a resumed durable
    ...
    hops workers in the waterfall (the worker-attribution goal) instead of overwriting history.
...
* ``node_io`` — per-node I/O context, lazy-loaded on click, **NEVER exported**. Port-keyed
  ``inputs``/``outputs`` (named in-ports — one entry per port). ``UNIQUE(run_id, node_id)`` is the
  idempotency guard. ``capture_level`` records what was actually persisted so ``/io`` can report
  ``full`` / ``metadata`` (sizes only) / ``off`` honestly.
* ``agent_io_policy`` — per-agent capture governance, keyed to the **stable** ``agent.id``,
  NOT ``agent_version`` — so editing capture policy never changes ``contentHash`` (agent version
...
def upgrade() -> None:
    op.create_table(
        "span",
        # Deterministic composite id (idempotency): {run_id}:{node_id}[:{phase}][:#{branch}].
        sa.Column("id", sa.String(), primary_key=True),
        sa.Column("run_id", sa.String(), sa.ForeignKey("run.id"), nullable=False),
        # OTel ids (hex) — for joining to an external backend if OTLP export is configured.
        ...
        # Epoch nanoseconds — clean integer arithmetic for durations + gaps.
        ...
        # Worker attribution: which durable worker/executor handled this span.
        sa.Column("executor_id", sa.String(), nullable=True),
        sa.Column("worker_host", sa.String(), nullable=True),
        # Monotonic per run — the ordering key (not ULID/time).
    ...
    # Idempotency guard: one I/O row per (run, node), upsert/DO-NOTHING on a replay re-write.
    op.create_index("ix_node_io_run_node", "node_io", ["run_id", "node_id"], unique=True)
    op.create_index("ix_node_io_run", "node_io", ["run_id"])

    op.create_table(
        "agent_io_policy",
        # Keyed to the STABLE agent id — NOT agent_version, so editing it never changes
