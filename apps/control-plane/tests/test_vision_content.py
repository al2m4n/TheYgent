"""Fast suite for multimodal ``llm`` content — the M20 cross-plane follow-up (m20.md).

M20 made the inference plane serve vision locally (mlx_vlm on ``/v1/chat/completions``), but a
...
what crossed the wire — exactly the §9.1 OpenAI shape LiteLLM forwards to the engine).
...
What they pin:
  ...
  * a plain-string content is untouched (the pre-M20 path still works alongside the new one).
"""
...
def test_vision_logical_model_id_is_forwarded(
    ...
    # The binding's logical id (a vision model) reaches the seam unchanged — the §8.4/§9.1.1
...
def test_string_content_still_works_alongside_multimodal(
    ...
    # The pre-M20 string path is untouched: a plain-string content still renders to a plain string
